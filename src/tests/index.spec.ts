...
describe('Test endpoint response', () => {
  ...
  it('test app endpoint', async () => {
    ...
    expect(response.text).toBe('Welcom to Image Prossesing App..... use api/images?filename={yourfilename} to get started');
  });
  ...
  it('test API endpoint with no parameters', async () => {
    ...
    expect(response.text).toBe(
      'Please provide a valid filename as a query parameter'
    );
  });
  ...
  it('Test resize API', async () => {
    ...
    expect(fs.existsSync(path.join(await getDirectory(__dirname), 'thumbnail', img))
    ).toBeTruthy();
  });
